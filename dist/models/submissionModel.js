...
class Submission {
    ...
    static async create(type, data, contactName, contactEmail, contactPhone) {
        await ensureTable();
        const [result] = await database_js_1.default.query(`INSERT INTO ResourceSubmissions
         (SubmissionType, SubmissionData, ContactName, ContactEmail, ContactPhone)
       VALUES (?, ?, ?, ?, ?)`, [type, JSON.stringify(data), contactName || null, contactEmail || null, contactPhone || null]);
        return result.insertId;
    }
    /** Fetch all pending submissions for the admin dashboard */
    static async findPending() {
        await ensureTable();
        const [rows] = await database_js_1.default.query(`SELECT SubmissionId, SubmissionType, SubmissionData, ContactName, ContactEmail, ContactPhone, SubmittedAt
