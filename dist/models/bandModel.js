...
export default class Band {
    static async findAll() {
        const [rows] = await pool.query('SELECT BandNumber, BandName, PictureURL FROM Bands ORDER BY BandName ASC');
        return rows;
    }
    ...
    static async countAll() {
        const [rows] = await pool.query('SELECT COUNT(*) as total FROM Bands WHERE Active = 1');
        return rows[0].total;
    }
    static async findByLetter(letter) {
        const [rows] = await pool.query('SELECT BandNumber, BandName, PictureURL FROM Bands WHERE BandName LIKE ? ORDER BY BandName ASC', [`${letter}%`]);
        return rows;
    }
    ...
    static async findById(bandNumber) {
        ...
        const [rows] = await pool.query('SELECT BandNumber, BandName, BandWebsite, PictureURL FROM Bands WHERE BandNumber = ?', [bandNumber]);
        return rows[0] || null;
    }
