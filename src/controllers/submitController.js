...
export async function submitFestival(req, res) {
  ...
  const renderForm = (error) =>
    res.render('submit/festival', { title: 'Submit a Festival', success: false, error, formData: req.body });

  if (!FestivalName || !StartDate || !EndDate || !City || !State) return renderForm('Please fill in all required fields.');
  ...
  try {
    await Submission.create('festival', data, ContactName, ContactEmail, ContactPhone || null);
    await sendSubmissionNotification('festival', data, { ContactName, ContactEmail, ContactPhone });
...
export async function submitBand(req, res) {
  ...
  const renderForm = (error) =>
    res.render('submit/band', { title: 'Submit a Band / Tour Update', success: false, error, formData: req.body });

  if (!BandName || !SubmissionType) return renderForm('Please fill in all required fields.');
  if (!ContactName || !ContactEmail) return renderForm('Contact name and email are required.');
  if (!emailRegex.test(ContactEmail)) return renderForm('Please enter a valid email address.');
  if (!AuthorizedToSubmit) return renderForm('You must confirm authorization to submit.');
  ...
  try {
    await Submission.create('band', data, ContactName, ContactEmail, ContactPhone || null);
    await sendSubmissionNotification('band', data, { ContactName, ContactEmail, ContactPhone });
...
export async function submitCamp(req, res) {
  ...
  const renderForm = (error) =>
    res.render('submit/camp', { title: 'Submit a Camp / Workshop', success: false, error, formData: req.body });

  if (!EventName || !StartDate || !EndDate || !City || !State) return renderForm('Please fill in all required fields.');
  if (!ContactName || !ContactEmail) return renderForm('Contact name and email are required.');
  if (!emailRegex.test(ContactEmail)) return renderForm('Please enter a valid email address.');
  if (!AuthorizedToSubmit) return renderForm('You must confirm authorization to submit.');
  ...
  try {
    await Submission.create('camp', data, ContactName, ContactEmail, ContactPhone || null);
    await sendSubmissionNotification('camp', data, { ContactName, ContactEmail, ContactPhone });
...
export function showVenueForm(req, res) {
...
export async function submitVenue(req, res) {
  ...
  try {
    ...
    await sendSubmissionNotification('venue', data, { ContactName, ContactEmail, ContactPhone });
...
export function showPhotosForm(req, res) {
...
export async function submitPhotos(req, res) {
  ...
  const renderForm = (error) =>
    res.render('submit/photos', { title: 'Submit Photos', success: false, error, formData: req.body });

  if (!PhotoType || !RelatedName || !Caption) return renderForm('Please fill in all required fields.');
  if (!ContactName || !ContactEmail) return renderForm('Contact name and email are required.');
  if (!emailRegex.test(ContactEmail)) return renderForm('Please enter a valid email address.');
  if (!OwnsRights) return renderForm('You must confirm you own or have rights to this photo.');
  if (!AuthorizedToSubmit) return renderForm('You must confirm authorization to submit.');

  const data = { PhotoType, RelatedName, ImageURL: ImageURL || null, Caption };

  try {
    await Submission.create('photos', data, ContactName, ContactEmail, ContactPhone || null);
    await sendSubmissionNotification('photos', data, { ContactName, ContactEmail, ContactPhone });
...
export function showReportForm(req, res) {
...
export async function submitReport(req, res) {
  ...
  if (!PageURL || !PageURL.trim()) return renderForm('Please provide the page URL.');
  if (!Description || !Description.trim()) return renderForm('Please describe the problem.');
  if (!ContactName || !ContactEmail) return renderForm('Contact name and email are required.');
  if (!emailRegex.test(ContactEmail)) return renderForm('Please enter a valid email address.');
  if (!AuthorizedToSubmit) return renderForm('Please confirm your submission.');
  ...
  try {
    await Submission.create('report', data, ContactName, ContactEmail, ContactPhone || null);
    await sendSubmissionNotification('report', data, { ContactName, ContactEmail, ContactPhone });
    res.render('report/new', {
      title: 'Report a Problem',
      success: true,
      error: null,
