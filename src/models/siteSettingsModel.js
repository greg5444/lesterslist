...
export default class SiteSettings {
  ...
  static async get(key) {
    const [rows] = await pool.query(
      'SELECT setting_value, setting_type FROM SiteSettings WHERE setting_key = ?',
      [key]
    );
  ...
  static async getMultiple(keys) {
    const [rows] = await pool.query(
      'SELECT setting_key, setting_value, setting_type FROM SiteSettings WHERE setting_key IN (?)',
      [keys]
    );
  ...
  static async set(key, value, updatedBy = 'admin') {
    await pool.query(
      `INSERT INTO SiteSettings (setting_key, setting_value, updated_by) 
       VALUES (?, ?, ?)
